...
$r   = $conn->query("SELECT id, name FROM doctors WHERE reset_token='$token' AND reset_expires > '$now' LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $valid) {
    ...
    if (strlen($password) < 6) {
        ...
        $id     = (int)$patient['id'];
        $conn->query("UPDATE doctors SET password='$hashed', reset_token=NULL, reset_expires=NULL WHERE id=$id");
  ...
  <div style="background:#fff;border-radius:16px;padding:36px">
   <?php if ($success): ?>
    <div style="text-align:center">
     ...
     <a href="login.php" class="btn-t btn-full" style="padding:13px;font-size:14px;border-radius:10px;text-decoration:none;display:inline-block;box-sizing:border-box">Log in with new password &rarr;</a>
    </div>
   <?php elseif (!$valid): ?>
    <div style="text-align:center">
     <div style="font-size:48px;margin-bottom:16px">&#9200;</div>
     ...
     <a href="forgot_password.php" class="btn-t btn-full" style="padding:13px;font-size:14px;border-radius:10px;text-decoration:none;display:inline-block;box-sizing:border-box">Request a new link</a>
    </div>
   <?php else: ?>
    ...
    <form method="POST" action="">
     ...
     <button type="submit" class="btn-t btn-full" style="padding:13px;font-size:14px;border-radius:10px;margin-bottom:14px">Reset password</button>
    </form>
    <p style="text-align:center;font-size:13px;color:var(--tx3)"><a href="login.php" style="color:var(--t);font-weight:700;text-decoration:none">&larr; Back to login</a></p>
   <?php endif; ?>
  </div>
 </div>
</div>
